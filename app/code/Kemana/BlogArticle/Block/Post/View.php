...
use Kemana\BlogArticle\Model\Post;
 ...
 */
class View extends \Kemana\Blog\Block\Listpost
{
    /**
     * config logo blog path
     */
    const LOGO = 'kemana/blog/logo/';
     ...
     * @return string
     */
    public function getTopicList($post)
    {
        $topicCollection = $post->getSelectedTopicsCollection();
        $result = '';
        if (!empty($topicCollection)) {
            $listTopics = [];
            foreach ($topicCollection as $topic) {
                $listTopics[] = '<a class="mp-info" href="' . $this->getTopicUrl($topic) . '">' . $topic->getName() . '</a>';
            }
            $result = implode(', ', $listTopics);
        }

        if ($result) {
            return $result;
        }
        return "tidak ada topic";
    }
