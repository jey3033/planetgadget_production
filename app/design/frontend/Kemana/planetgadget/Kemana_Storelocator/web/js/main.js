    ...
    $.widget('mage.ksLocator', {
        ...
        navigateMe: function(){
            ...
            if ( (navigator.geolocation) && (this.useGeo==1) ) {
                ...
                }, this.navigateFail, $('.distance-away').hide() );
            }else{
                this.makeAjaxCall();
            }
        },
        ...
        Kemanaload: function(response) {
            ...
            $("#" + this.options.storeListId + " .today_schedule" ).click(function(event) {
                $(this).next( ".all_schedule" ).slideToggle( "slow", function() {
                    // Animation complete.
                }).toggleClass('toggled');
        ...
        processLocation: function(locations) {
            let self = this;
            var template = baloonTemplate.baloon; // document.getElementById("kemana_window_template").innerHTML;
            var curtemplate = "";
            ...
            for (var i = 0; i < locations.totalRecords; i++) {
                ...
                if (self.options.infoWindowEnabled) {
            ...
            this.marker[this.options.mapId].forEach((marker)=>{
        ...
        gotoPoint: function(myPoint) {
            ...
            const currentMarker = this.marker[mapId].find( ({locationId}) => locationId === myPoint );
            const {position:{lat,lng} = {}} = currentMarker;
            $('[data-mapid=' + mapId + ']').removeClass('active');
            $('[data-mapid=' + mapId + '][data-amid=' + myPoint + ']').addClass('active');
            if(currentMarker){
                this.map[mapId].setCenter(new google.maps.LatLng(lat(),lng()));
                this.map[mapId].setZoom(mapZoom);
                if (self.options.infoWindowEnabled) {
                    currentMarker['infowindow'].open(this.map[mapId], currentMarker);
                }
            }
        },
        ...
        createMarker: function(lat, lon, html, marker, locationId) {
            ...
            this.marker[this.options.mapId].push(newmarker);
        },
