define(["jquery", "utility", "slick", "mage/accordion", "domReady!"], function (
  $,
  utility,
  slick
) {
  ...
  $(window).resize(function () {
    if ($(document.body).width() < 767) {
      $(".main.actions .mobile-cart-buttons").insertAfter(
        $(".cart-container .form-cart")
      );
    } else {
      $(".cart-container .mobile-cart-buttons").insertBefore(
  ...
  function moveClearButton() {
    $(function () {
      if ($(document.body).width() > 767)
        $(".mobile-cart-buttons").after($(".cart.actions .action.clear"));
    });
  }
  ...
  function setNoReviewSection() {
    if ($(".product-reviews-summary").hasClass("empty")) {
      $(".product-reviews-summary").prepend(
  ...
  function setMyAccountMobileNav() {
    if (utility.isMobile()) {
      ...
      let actionContent =
        '<li class="nav item action"><strong>' + currentText + "</strong></li>";
  ...
  function setMyOrderNavigation() {
    if (utility.isMobile()) {
      ...
      $(".order-links").prepend(
        '<li class="nav item action"><strong>' + text + "</strong></li>"
      );
    }
  }
   ...
   * 18.Product Widget slider
   */
  if ($(".products-related")[0] || $(".products-upsell")[0]) {
  ...
  $(".level1.category-item > a").mouseover(function () {
    if (!$(this).parent().find(".megamenu-inner").length > 0) {
      $(".megamenu-wrapper.tabs-menu.vertical").attr(
        "style",
